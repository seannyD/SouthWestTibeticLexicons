...
trees = read.nexus(file="../results/BEAST/jml_combined_relaxed_multisite_fossilised/JMLwithSagart_combined_multisite.trees")
...
for(i in 1:length(trees)){
  ...
  SWTibToBatang = max(nodeHeights(tx))
  tx2 = keep.tip(trees[[i]],c("Lhasa","Xiahe","Alike"))
  XAToBatang = max(nodeHeights(tx2))
...
res.cadm.max <- CADM.global(cost.geo.vs.phylo,2,ncol(geoDists),nperm = 10000) res.cadm.max
...
trees = read.nexus("../results/BEAST/jml_combined_relaxed_multisite_fossilised/JMLwithSagart_combined_multisite.trees")
...
for(i in 1:length(trees)){
  ...
  phyDistsD = as.matrix(as.dist(phyDists))
  cost.geo.vs.phylo <- rbind(geoDistsD,phyDistsD)
  res.cadm.max <- CADM.global(cost.geo.vs.phylo,2,ncol(geoDists),nperm = 1000)
...
min(res$W)
max(res$Prob.perm)
write.csv(res,"../results/BEAST/jml_combined_relaxed_multisite_fossilised/GeoPhyloDistances_GreatCircle.csv")
