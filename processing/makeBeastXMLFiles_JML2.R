...
### Compare with lexical-level conding
dl = read.csv("../data/corrected/AllLangs_240_CogID_Sagart_Complete_Extended_Checked4.tsv", sep="\t",comment.char = "#",encoding = "UTF-8",fileEncoding = "UTF-8",quote = "",stringsAsFactors = F)

snm = data.frame()
langs = unique(d$DOCULECT)
# langs = c("OldTibetan","Xiahe", "Alike", "Lhasa", "Batang")
for(concept in unique(d$CONCEPT)){
  if(concept %in% dl$CONCEPT){
    dx = d[d$CONCEPT==concept,]
    dlx = dl[dl$CONCEPT == concept,]
    dlx = dlx[dlx$DOCULECT %in% unique(dx$DOCULECT),]
    cog2 = dx[match(langs,dx$DOCULECT),]$COGID
    cog1 = dlx[match(langs,dlx$DOCULECT),]$COGID
    keep = !(is.na(cog1) | is.na(cog2))
    cog1 = cog1[keep]
    cog2 = cog2[keep]
    tx = table(cog1,cog2)
    split1 = sum(apply(tx,1,function(X){sum(X>0)})>1) # split
    split2 = 0
    if(split1==0){
      split2 = length(unique(cog1))!=length(unique(cog2))
    }
    splits = split1+split2
    mergers = sum(apply(tx,2,function(X){sum(X>0)})>1) # merger
    snm = rbind(snm, data.frame(
      concept = concept,
      splits = splits,
      mergers = mergers
    ))
...
sum(snm$splits>0 | snm$mergers>0)/nrow(snm)
sum(snm$splits)+ sum(snm$mergers)
